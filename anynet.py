...
#             self.insert_child_to_cell("b{}".format(i + 1), block)
#             stride, w_in = 1, w_out
#
#     def construct(self, x):
...
#             self.insert_child_to_cell("s{}".format(i + 1), stage)
#             prev_w = w
...
class AnyNet(Cell):
    ...
    def __init__(self, params=None):
        ...
        for i, (d, w, s, b, g) in enumerate(zip(*[p[k] for k in keys])):
            ...
            prev_w = w
        self.head = AnyHead(prev_w, p["head_w"], p["num_classes"])
        # for i in self.stages.cells_and_names():
        #     print(i[1])
        # break
        # for i in self.head.cells_and_names():
        #     print(i[0])
        # break
